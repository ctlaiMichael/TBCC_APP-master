...
import { F2100202ResBody } from './f2100202-res';
import { F2100202ReqBody } from './f2100202-req';
...
@Injectable()
export class F2100202ApiService extends ApiBase<F2100202ReqBody, F2100202ResBody> {
    constructor(
        public telegram: TelegramService<F2100202ResBody>,
        public errorHandler: HandleErrorService,
        public authService: AuthService,
        private _logger: Logger,
        private _formateService: FormateService
    ) {
        super(telegram, errorHandler, 'F2100202');
    }
  ...
  getData(req: object, page?: number, sort?: Array<any>, pageSize?: string | number): Promise<any> {
        ...
        // 參數處理
        let data: F2100202ReqBody = new F2100202ReqBody();
        data.paginator = this.modifyPageReq(data.paginator, page, sort);
    if (pageSize) {
      data.paginator.pageSize = pageSize;
    }
        data.custId = userData.custId; // user info;
        data.acctNo = this._formateService.checkField(req, 'acctNo'); // 從api F2000201取得
        data.acctType = this._formateService.checkField(req, 'acctType'); // 從api F2000201取得
    ...
    private _modifyRespose(resObj) {
        ...
        // other data
        output.summary_data = ContentSummaryApiUtil.modifyForexTime(resObj, 'F2100202');
        return output;
    }
