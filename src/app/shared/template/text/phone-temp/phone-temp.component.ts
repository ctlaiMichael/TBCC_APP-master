...
@Component({
    selector: 'app-phone-temp',
    templateUrl: './phone-temp.component.html',
    styleUrls: [],

})
export class PhoneTempComponent implements OnInit, OnChanges {
    /**
     * 參數處理
     */
    @Input() setData: any;
    ...
    title = 'FIELD.PHONE'; // 手機號碼
    placeholderStr = 'FIELD.PHONE_EDIT'; // 請輸入手機號碼
    inp_data = '';
    error_data = '';
    disabledFlag = false;
    requireFlag = false;
    ...
    onBackPageData(e, type?: string) {
        if (type === 'change') {
            // change event
            this._logger.step('Phone', 'change', this.inp_data);
            // 檢查密碼格式
            this.onCheckEvent();
        } else {
            // module change (focuse in)
            this._logger.step('Phone', 'model change', this.inp_data);
        }
        if (!this.disabledFlag) {
            // disabled不可回傳
    ...
    onCheckEvent() {
        ...
        if (do_check) {
            const check_data = UserCheckUtil.checkMobile(this.inp_data);
