...
@Component({
    selector: 'app-loan-detail-mainpage',
    templateUrl: './loan-detail-mainpage.component.html',
    styleUrls: []

})
export class LoanDetailMainComponent implements OnInit {
    /**
     * 參數處理
     */
    @Input() acctObj: any;
    ...
    onScrollEvent(next_page) {
        ...
        let componentRef: any = this.paginatorCtrl.addPages(appendBox, LoanDetailPaginatorComponent);
        ...
        switch (page) {
            case 'list-item':
                ...
                if (this.pageCounter == 1) {
                    ...
                    if (errorObj.hasOwnProperty('data') && errorObj['data'] && errorObj['data'].hasOwnProperty('info_data')) {
                        // 借款資料顯示(無交易明細仍要顯示)
                        this.info_data = errorObj['data']['info_data'];
                    }
    ...
    */
    ...
    private _initEvent() {
        ...
        // 查詢頁面說明事項
        this.searchInfoData = [
            // 輸入查詢之起始日期及終止日期，借款資料可查詢近兩年內資料。
            'PG_CREDIT.LOAN.SEARCH_INFO.INFO1'
        ];
    }
