...
export class QRTpyeCardService {
  PayType: any;
  emv = '0';
  returnObj: any;
  isTwpay: any;
  ...
  async getSecurityInfo(stepObj, securityType: any): Promise<any> {
    ...
    logger.info('Epay-card', 'security start', this._formateService.transClone(securityType), this._formateService.transClone(stepObj));
    let security_key = this._formateService.checkField(securityType, 'key');
    logger.info('security_key',security_key);
    let modify_security = this.securityDataModify(security_key, stepObj);
    logger.info('modify_security',modify_security);
    ...
    try {
      ...
      logger.info('trnsToken',  trnsToken);
      if (!trnsToken) {
      ...
      return new Promise((resolve, reject) => {
        if (modify_security.key === 'Biometric') {
          ...
          logger.info('send doSecurityNextStep start', set_security);
          this.check_security.doSecurityNextStep(set_security).then(
            // tslint:disable-next-line: no-shadowed-variable
            (resSecurity) => {
              this._logger.step('Epay-card', 'send security success', resSecurity);
   ...
   */
  checkBiometric() {
    return this.auth.checkBiometric_card();
  }
  ...
  private getSecurityList(set_data?: string | Array<string>) {
    logger.error('getSecurityList set_data',set_data);
    let output: any = {
      status: false,
      securityTypes: [],
    ...
    if (typeof set_data !== 'undefined') {
      ...
      logger.error('allow_list',allow_list);
      // 特殊開關設定
      let tk: any;
    ...
    logger.info('checkBiometric',checkIsBio);
    logger.info('checkBiometric2',securityList[set_security_key].status);
    if (checkIsBio) {
    ...
    logger.info('securityTypes 4 over',securityTypes);
    // 圖形密碼 
    set_security_key = 5;
    let tempRem = this.localStorage.getObj('Remember');
    ...
    //信卡會員 第一次登入
    if(!tempRem && this.session.getObj("login_method") == '2'){
      // 登入資訊
      tempRem = {
        userData: {
          custId: '',
    ...
    let tempRem_card = this.localStorage.getObj('Remember_card');
    let ftUser_card = this.localStorage.getObj('Compare_card');
    if ((tempRem.ftlogin.payPattern === '1') &&
      (Number(tempRem.ftlogin.patterLoginErrorCount) < Number(this.maxPatterLockError)) &&
      (ftUser_card.comparecustId === tempRem_card.userData.custId) &&
    ...
    logger.error(output.securityTypes);
    return output;
  }
  ...
  getSecurityData(option?: object) {
    ...
    // securityTypes = this.securityServicesGet();//不取cache信用卡會員 
