...
@Injectable()
export class OnscanCardService {
    ...
    initScan(): Promise<any> {
        this._logger.step('Epay-card', 'onscan init in service');
        // 查看上次交易狀態沒有的話給預設值
        let old_data = this.localStorage.get('lastTransaction');
        if (old_data === false || old_data == '') {
            this._logger.step('Epay-card', 'lastTransaction set 1');
    ...
    changePage(url, params?: any) {
        ...
        this.closeScan().then(
            (closeResObj) => {
                this._logger.step('Epay-card', 'cancelScan an go', closeResObj, url, params);
    ...
    closeScan(): Promise<any> {
        if (!!sessionStorage.cameraReady || !!this.openCaramsFlag) {
            this._logger.step('Epay-card', 'do cancel Scan', sessionStorage.cameraReady, this.openCaramsFlag);
            logger.error('close5');
            this.openCaramsFlag = false;
            delete sessionStorage.cameraReady;
            return this.scan.closeCamera().then(
                (closeResObj) => {
                    this._logger.step('Epay-card', 'cancelScan', closeResObj);
            ...
            this._logger.step('Epay-card', 'cancelScan', 'no do');
    ...
    checkError(resultObj) {
        this._logger.step('Epay-card', 'checkError', resultObj);
        ...
        if (isCancel || msg_code == 'SCAN_CANCEL') {
            ...
            this.handleError.handleError(output).then(
                () => {
                    this.changePage('epay-card');
                }
            );
        } else {
            let back_path = 'epay-card'; // 錯誤固定返回選單
    ...
    async openScan(): Promise<any> {
        ...
        this._logger.step('Epay-card', 'open init');
        this.resetScanData();
        this.uiContent.setSectionScroll(false); // 停止頁面滾動
        this.scan.waitForReady().then(() => {
            if (!sessionStorage.cameraReady) {
                this._logger.step('Epay-card', 'open 2: sessionStorage.cameraReady set');
                sessionStorage.cameraReady = true;
            }
        }).catch((errorObj) => {
            this._logger.step('Epay-card', 'open 2: camera not open', errorObj);
        });
        this._logger.step('Epay-card', 'open 3: openCamera');
        try {
            sessionStorage.cameraReady = true;
            let Qrcode_res = await this.scan.openCamera();
            // this._logger.step('Epay-card', 'open 4: analyze ready', Qrcode_res);
    ...
    async imageScan(): Promise<any> {
        this._logger.step('Epay-card', 'imageScan start');
        this.resetScanData();
        try {
    ...
    async analyzeCode(Qrcode_res): Promise<any> {
        this._logger.step('Epay-card', 'analyze init', Qrcode_res);
        try {
    ...
    private async modifyAnalyzeReq(Qrcode_res): Promise<any> {
        ...
        if(!environment.NATIVE){//test
            qrcodeType= 'EMV';
        }
        this._logger.step('Epay-card', 'analyze ', qrcodeType);
        logger.error('analyze qrcodeType',qrcodeType);
        if (qrcodeType === 'TWQRP') {
            /**
             * TWQRP規格
             */
            ...
            if (resfq000102.need104Flag) {
                ...
                this.navgator.push('cardlogin-qrCodePayCardForm', {
                    trnsfrOutCard: '',
                    qrcode: resfq000102
                });
                // 後續執行取消交易作業
    ...
    private modifyAnalyzeEmv(preCheckQrcode, jsonObj): Promise<any> {
        this._logger.step('Epay-card', 'analyze EMV');
        let req: any = {
            'EMVQRCode': preCheckQrcode
        };
        return this.fq000102.send(req).then(
            (resfq000102) => {
                if(resfq000102['_qrType']&&resfq000102['_qrType']=='3'){
                    if (resfq000102['_qrType'] && resfq000102['_qrType'] == '3') {
    ...
    private processAnalyze(analyzeCode: QrcodeAnalyzeOptions): Promise<any> {
        ...
        // 加入判斷繳費與繳稅
        this._logger.step('Epay-card', 'analyzeCode', analyzeCode);
    ...
    private checkQRCode(analyzeCode: QrcodeAnalyzeOptions): QrcodeFieldsOptions {
        ...
        if (!this._checkService.checkEmpty(qrcodeData, true)) {
            this._logger.step('Epay-card', 'Error empty', qrcodeData);
        ...
        this._logger.step('Epay-card', 'checkQrcode', output);
        return output;
    }
