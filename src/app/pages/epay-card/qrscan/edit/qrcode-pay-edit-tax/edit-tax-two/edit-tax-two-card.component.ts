...
@Component({
  selector: 'app-edit-tax-two-card',
  templateUrl: './edit-tax-two-card.component.html',
  styleUrls: ['./edit-tax-two-card.component.css']
})
export class EditTaxTwoCardComponent implements OnInit {
  ...
  _init() {
    ...
    // this.clickAccts();
    // this.clickCard();
    this.status = 3; //預設信用卡，且直接發fc001001查詢信用卡帳號
    this.clickCard();
  }
  chgStatus(status) {
    // if (status == '2') {
  ...
  //     res4000101 => {
  //       this._logger.step('Epay-card', '測試res4000101: ', res4000101);
  ...
  //         );
  //       }
  //       this._logger.step('Epay-card', '測試res4000101: ', res4000101.body);
  ...
  //       }
  //       this._logger.step('Epay-card', '測試accts: ', this.accts);
  //       this.accts.forEach(element => {
  //         this._logger.step('Epay-card', '測試印出element:', element.acctNo);
  //       });
  //     },
  //     error => {
  ...
  clickSubmit() {
    ...
    this._logger.log("into clickSubmit, card:", this.card);
    this._logger.log("into clickSubmit, form.cardDate:", this.form.cardDate);
    this._logger.log("into clickSubmit, form.cardPassword:", this.form.cardPassword);
    let checkDateNumber = this._errorCheck.checkNumber(this.form.cardDate); //有效年月
    let checkPassNumber = this._errorCheck.checkNumber(this.form.cardPassword); //卡背末3碼
    this._logger.log("checkDateNumber:", checkDateNumber);
    this._logger.log("checkPassNumber:", checkPassNumber);
    // this._logger.step('Epay-card', '測試印出params:', params);
    // this._logger.step('Epay-card', '測試印出status:', this.status);
    // this._logger.step('Epay-card', '測試印出accts:', this.accts);
    // this._logger.step('Epay-card', '測試印出cardds:', this.cardds);
    // this._logger.step('Epay-card', '測試印出card:', this.card);
    // this._logger.step('Epay-card', '測試印出cardDate:', this.form.cardDate);
    // this._logger.step('Epay-card', '測試印出cardPassword:', this.form.cardPassword);
    // this._logger.step('Epay-card', '測試印出life:', this.life);
    // this._logger.step('Epay-card', '測試印出form.trnsfrOutAcct:', this.form.trnsfrOutAcct);
    // this._logger.step('Epay-card', '測試印出acts:', this.acts);
    // this._logger.step('Epay-card', '開始');
    // 沒有選擇繳款方式情況
    if (this.status === null) {
      this._logger.step('Epay-card', '請選擇繳款方式');
      this.alert.show('請選擇繳款方式');
    } else if (this.status == 2 && this.accts === null) {
      // 活期
      this._logger.step('Epay-card', '您目前尚未設定約定轉出帳戶，請改以其它方式繳款');
      this.alert.show('您目前尚未設定約定轉出帳戶，請改以其它方式繳款');
    } else if (this.status == 2 && this.accts !== null && this.life === null) {
      this._logger.step('Epay-card', '請選擇活期存款帳戶');
      this.alert.show('請選擇活期存款帳戶');
    } else if (this.status == 3 && this.cardds === null) {
      // 信用卡
      this._logger.step('Epay-card', '您目前無本行信用卡，請改以其它方式繳款');
      this.alert.show('您目前無本行信用卡，請改以其它方式繳款');
    } else if (this.status == 3 && this.cardds !== null && this.card == '') {
      this._logger.step('Epay-card', '請選擇信用卡');
      this.alert.show('請選擇信用卡');
      // tslint:disable-next-line: max-line-length
    } else if (this.status == 3 && this.cardds !== null && this.card !== null && (this.form.cardDate == null || this.form.cardDate == '' || typeof this.form.cardDate == 'undefined')) {
      this._logger.step('Epay-card', '請輸入信用卡有效月年:');
      this.errorMsg.cardDate = '請輸入信用卡有效月年';
      // tslint:disable-next-line: max-line-length
    } else if (this.status == 3 && this.cardds !== null && this.card !== null && (this.form.cardPassword == null || this.form.cardPassword == '' || typeof this.form.cardPassword == 'undefined')) {
      this._logger.step('Epay-card', '請輸入信用卡背面末三碼');
    ...
    //增加判斷輸入為數字，才給進行交易
      ...
      && this.errorMsg.cardDate == '' && this.errorMsg.cardPassword == '') {
      this._logger.step('Epay-card', '執行FQ000201');
      // 執行FQ000201
      ...
      this.confirm.show(message).then(
        reschecked => {
          this._logger.step('Epay-card', '測試印出FQ000201:', reschecked);
          ...
          this._logger.step('Epay-card', '測試印出CA_Object:', CA_Object);
          ...
          this.fq000201.send(respon).then(
            (resfq000201) => {
              this._logger.step('Epay-card', '測試印出resfq000201:', resfq000201);
              ...
              this._logger.step('Epay-card', '測試印出resfq000201:', params);
    ...
    //交易電文 活儲交易(非信用卡交易隱藏) status == 3 才是信用卡
    ...
    //   }
    //   this._logger.step('Epay-card', '檢查執行FQ000202(formObj): ', formObj);
    ...
    //   this._logger.step('Epay-card', '檢查執行FQ000202(respon): ', respon);
    //   const CA_Object = {
    //     securityType: '',
    //     serviceId: 'FQ000202',
    //     signText: respon,
    ...
    //             detail: 'qrcodePayTaxResult'
    //           };
    //           this._logger.step('Epay-card', '測試印出resfq000202:', params);
    //           this.navgator.push('cardlogin-qrcodePayResult', params);
    //         }).
    //         catch(
    //           error => {
    ...
    //   );
    // }
    // this._logger.step('Epay-card', '不做任何事');
  }
  clickCancel() {
    this.navgator.push('epay-card');
  }
