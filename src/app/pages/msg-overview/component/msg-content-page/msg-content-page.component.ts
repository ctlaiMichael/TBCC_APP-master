...
@Component({
  selector: 'app-msg-content-page',
  templateUrl: './msg-content-page.component.html',
})
export class MsgContentPageComponent implements OnInit {
  ...
  ngOnInit() {
    this._logger.log(`contentObj = ${JSON.stringify(this.contentObj)}`);
    if (!!this.contentObj['DataTime'] && typeof this.contentObj['DataTime'] == 'string') {
      this.contentObj['DataTime'] = String(this.contentObj['DataTime']).split(' ')[0];
    }
    // 取得(url)
