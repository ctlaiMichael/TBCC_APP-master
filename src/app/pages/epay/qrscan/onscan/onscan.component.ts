...
@Component({
  selector: 'app-onscan',
  templateUrl: './onscan.component.html',
  styleUrls: ['./onscan.component.css'],
  providers: [OnscanService]
})
export class OnscanComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  constructor(
    ...
    private _mainService: OnscanService,
  ...
  ngOnInit() {
    ...
    this._logger.step('Epay', 'onscan init');
    ...
    this._mainService.initScan().then(
      ...
      (errorObj) => {
        ...
        this._mainService.changePage('epay');
      }
    );
  }
   ...
   */
  ngOnDestroy() {
    this._logger.step('Epay', 'onscan destroy');
  ...
  async scanFromLib(decQRCode?: string) {
    this._logger.step('Epay', 'scanFromLib init');
    this.loading.show('Epay_analyze'); // 開啟解析中畫面
    try {
      let Qrcode_res = '';
      if (!decQRCode) {
      ...
      try {
        ...
        let resultObj = await this._mainService.analyzeCode(Qrcode_res);
        this._logger.step('Epay', 'qrcodeAnalyze[img]', resultObj);
        this.sameProcess(resultObj);
      } catch (errorObj) {
        this._logger.step('Epay', 'scanFromLib Analyze', errorObj);
        // this.open(); // 失敗必須重開相機
        this.loading.hide('Epay_analyze'); // 關閉解析中畫面
        this._mainService.checkError(errorObj);
      }
    } catch (imgError) {
      this._logger.step('Epay', 'scanFromLib Error', imgError);
      this.loading.hide('Epay_analyze'); // 關閉解析中畫面
  ...
  gotoBeScan() {
    ...
    // const checkIsBio = this.qrtype.checkBiometric();
    // if (checkIsBio) {
    ...
    //     cnEndDate = this.formateService.transDate(new Date());
    //   } else {
    //     cnEndDate = this.formateService.transDate(res.cnEndDate, 'date');
    //   }
    //   let todayDate = this.formateService.transDate(new Date());
        ...
        resSecurityInfo => {
          ...
          this.fq000302.send(resSecurityInfo.responseObj.signText, reqHeader).then(
            (resfq000302) => {
              ...
              this._mainService.changePage('qrcodeShowPay', params);
            })
            .catch(
              error => {
                this.clickBlock = false;
    ...
    this.fq000420.send(form).then(
      resObj => {
        ...
        if (res.cixp3Cid != 'Y' || res.cixP33Flg != 'Y') {
          ...
          this.navgator.push('outboundAgree', params);
        } else {
          // this._logger.error("into res.cixp3Cid == 'Y' || res.cixP33Flg == 'Y'");
          this.navgator.push('qrCodePayForm2', {
  ...
  sameProcess(resultObj) {
    ...
    //   }
    //   let todayDate = this.formateService.transDate(new Date());
    //   this._logger.step('Epay', 'cnEndDate', cnEndDate, DateUtil.compareDate(todayDate, cnEndDate));
    //   this._logger.step('Epay', 'resultObj', resultObj);
    //   if (res.cn === null || res.cn === '' || DateUtil.compareDate(todayDate, cnEndDate) === -1) {
    //   } else {
    ...
    if (securityTypes.length <= 0) {
      ...
      if (resultObj['data'].payCategory === '15001') {
        this.cancelScan();
        this.navgator.push('qrCodePayFormTax4', {
          trnsfrOutAcct: this.defaultTrnsOutAcct,
          trnsLimitAmt: this.trnsLimitAmt,
        ...
        this.cancelScan();
        this.navgator.push('qrCodePayFormTax2', {
          trnsfrOutAcct: this.defaultTrnsOutAcct,
          trnsLimitAmt: this.trnsLimitAmt,
          qrcode: resultObj['data'],
          webToAppRes: this.webToAppRes ? this.webToAppRes : ''
        ...
        this._logger.step('Epay', 'is pay-card');
        // 繳費網頁-繳卡費
        this.cancelScan();
        this.navgator.push('qrCodePayFormCard', {
          trnsfrOutAcct: this.defaultTrnsOutAcct,
          trnsLimitAmt: this.trnsLimitAmt,
        ...
        this._logger.step('Epay', 'is p2p (trnsType=02');
        // P2P轉帳
        this.cancelScan();
        this.navgator.push('qrCodeGetForm', {
          trnsfrOutAcct: this.defaultTrnsOutAcct,
          trnsLimitAmt: this.trnsLimitAmt,
          qrcode: resultObj['response'],
          webToAppRes: this.webToAppRes ? this.webToAppRes : ''
        ...
        this._logger.step('Epay', 'is 轉帳購物 (trnsType=01, acqInfo ==51,)');
        this.cancelScan();
        this.navgator.push('qrCodeBuyForm', {
          trnsfrOutAcct: this.defaultTrnsOutAcct,
          trnsLimitAmt: this.trnsLimitAmt,
          qrcode: resultObj['response'],
          webToAppRes: this.webToAppRes ? this.webToAppRes : ''
        ...
        this._logger.step('Epay', 'is outbound (acqInfo ==11,)');
        this.cancelScan();
        if (!(res.BoundID === '4' && res.OTPID === '2')) {
          // this._logger.error("into res.BoundID !== '4' && res.OTPID !== '2')");
          this.alert.show('您的行動裝置無(OTP)機制服務，請洽營業單位申請。').then(
            resAlert => {
              this._mainService.changePage('epay');
            }
          );
        } else {
          // 檢查是否完成身份認證與查詢聯徵P33程序
          this.getFQ000420();
        }
      } else {
        this._logger.step('Epay', 'is qrCodePayForm');
        // done

        this.cancelScan();
        this.navgator.push('qrCodePayForm', {
          trnsfrOutAcct: this.defaultTrnsOutAcct,
          trnsLimitAmt: this.trnsLimitAmt,
          qrcode: resultObj['response'],
          webToAppRes: this.webToAppRes ? this.webToAppRes : ''
