...
@Component({
	selector: 'app-edit-tax-four',
	templateUrl: './edit-tax-four.component.html',
	styleUrls: ['./edit-tax-four.component.css']
})
export class EditTaxFourComponent implements OnInit {
	...
	chgStatus() {
		...
		if (this.status == '1') {
			...
			this.disable1 ='';// 切回金融卡顯示確認鍵
			this.ismyself ='';// 清除點選是否資料
			this.clickOpen();
	...
	selectId(curr) {
			...
			if (curr == 'yes') {
				this.disable1 = '';
				//this.background1 = 'rgb(49,199,118)';
			} else if (curr == 'no') {
				this.disable1 = 'disable';
				// this.background1 = '#d0d3d6;';
				// this._handleError.handleError({
				// 	type: 'dialog',
				// 	title: '提醒您',
				// 	content: "信用卡限繳持卡人本人或配偶稅款。"
				// });
				this.alert.show('信用卡限繳持卡人本人或配偶稅款。');
				//return false;
			}
	...
	clickSubmit() {
		...
		} else if (this.status == '3' && this.ismyself !== 'yes') {
			this.alert.show('信用卡限繳持卡人本人或配偶稅款');
