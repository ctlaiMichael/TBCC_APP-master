...
export class CardManagementComponent implements OnInit {
  ...
  */
  ...
  _init() {
    ...
    // 20200604  pattern: false圖形手勢需求無綁定信用卡 ChengWei
    ...
    logger.error('selectSecurityType', this.selectSecurityType, this.securityTypes);
    // 無安控機制
    if (this.securityTypes.length < 1) {
    ...
    if (this.localstorage.get('nextAsk_aggreCard') !== '1' && this.localstorage.get('firstAggreCard') === '1') {
      this.confirmCheckbox.show('本人同意以快速交易、OTP或憑證作為交易密碼，並妥善保管。', { title: '注意事項', checkbox: '下次不要提醒我' }).then(
        (resConfirm) => {
  ...
  cardTelegram(method, card) {
    ...
    if (method == 'B' || method == 'A') {
      ...
      this.emvService.getSecurityInfo(CA_Object, this.selectSecurityType).then(
        (resSecurityInfo) => {
          logger.error('cardTelegram 402 method A', resSecurityInfo);

          let reqHeader = {
            header: resSecurityInfo.headerObj
          };

          logger.error('send 113', resSecurityInfo.responseObj.signText, reqObj);
          this.fq000113.send(reqObj, reqHeader).then(
            (res) => {
              ...
              if (res.body.trnsRsltCode == '0') {
                this.localstorage.set('lastTransaction', '2');
                this.navgator.push('cardBinding', params);
              } else {
                this.handleError.handleError({
                  type: 'message',
                  title: '設定失敗',
        ...
        (errorSecurityInfo) => {
          logger.error('errorSecurityInfo typeSB', errorSecurityInfo);
          //使用者取消
