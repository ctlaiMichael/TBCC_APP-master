...
export class PayCardComponent implements OnInit {
  ...
  async _init() {
    ...
    this.defaultTrnsOutAcct = trnsOutAcct;
    this.defaultTrnsCard = this.formateService.checkField(fq000101Data, 'defaultTrnsCard');
    //this.trnsfrOutCard = this.defaultTrnsCard;
    this.getCardType = this.defaultTrnsCard.substr(0, 1);
    //debugger;
    if (fq000101Data.trnsOutAccts == null
      || fq000101Data.trnsOutAccts.trnsOutAcct == null) {
    ...
    //debugger;
    // alert('typeAndFee' + this.form.typeAndFee);
    // alert('typeAndFeeValue' + this.form.typeAndFee.substr(1));
    // 判斷可使用繳費方式
    let Cardtype = this.defaultTrnsCard.substr(0, 1);
    //if (Cardtype == '4' || Cardtype == '3' || Cardtype == '2' || Cardtype == '5') { Cardtype = 'C'; }
    if (this.form.typeAndFee.indexOf('F') != '-1' && this.defaultTrnsCard == '') {
      this.typeAndFeeState1 = true;
      // tslint:disable-next-line:max-line-length
    } else if (this.form.typeAndFee.indexOf('F') != '-1' && this.defaultTrnsCard != '' && this.form.typeAndFee.indexOf('F') != '-1' && this.form.typeAndFee.indexOf('C') == '-1') {
      this.typeAndFeeState1 = true;
      // tslint:disable-next-line:max-line-length
    ...
    this._logger.step('Epay', 'security:', this.securityTypes, this.selectSecurityType);
   ...
   */
  onCheckEvent() {
    this._logger.step('Epay', 'onCheckEvent');
  ...
  onSaveEvent() {
    this._mainService.sendData(this.form, this.selectSecurityType, this.meansTransactionCard, this.defaultTrnsCard).then(
      (resData) => {
        this._logger.step('Epay', 'res go', resData);
