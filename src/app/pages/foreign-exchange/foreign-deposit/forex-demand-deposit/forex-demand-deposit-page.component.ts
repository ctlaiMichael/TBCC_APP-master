...
@Component({
    selector: 'app-forex-demand-deposit-page',
    templateUrl: './forex-demand-deposit-page.component.html',
    styleUrls: [],

})
export class ForexDemandDepositPageComponent implements OnInit {
    /**
     * 參數處理
     */
    @Input() acctObj: any;
    ...
    @ViewChild('pageBox1', { read: ViewContainerRef }) pageBox1: ViewContainerRef;
    @ViewChild('pageBox2', { read: ViewContainerRef }) pageBox2: ViewContainerRef;
    @ViewChild('pageBox3', { read: ViewContainerRef }) pageBox3: ViewContainerRef;
    // -- 分頁end -- //
    ...
    onScrollEvent(next_page) {
        ...
        switch (this.nowPageType) {
            case '1M': appendBox = this.pageBox1; break;
            case '3M': appendBox = this.pageBox2; break;
            case '6M': appendBox = this.pageBox3; break;
        ...
        let componentRef: any = this.paginatorCtrl.addPages(appendBox, ForexDemandPaginatorComponent);
    ...
    onErrorBackEvent(e) {
        ...
        switch (page) {
            case 'list-item':
                ...
                if (this.pageCounter == 1) {
                    // 列表頁：首次近來錯誤推頁
                    if (errorObj && errorObj.hasOwnProperty('data')) {
                        if (errorObj['data'].hasOwnProperty('dataTime') && errorObj['data']['summary_data']) {
                            this.itemData = errorObj['data']['summary_data'];
                        }
    ...
    */
    ...
    private _initEvent() {
        ...
        this.bookmarkData = this._mainService.getBookmark(this.acctObj);
        // 查詢頁面條件
        this.searchBoxRule = this._mainService.getDateSet('custom');
        // 查詢頁面說明事項
        this.searchInfoData = [
            // 輸入查詢之起始日期及終止日期，外幣活期性存款可查詢本月及六個月內資料。
            // 4/24/需求變更可查詢一年之資料(活期僅可查6個月)
            'PG_DEPOSIT.FOREX_DEPOSIT.SEARCH_INFO.INFO1','PG_DEPOSIT.FOREX_DEPOSIT.SEARCH_INFO.INFO3'
        ];
    }
