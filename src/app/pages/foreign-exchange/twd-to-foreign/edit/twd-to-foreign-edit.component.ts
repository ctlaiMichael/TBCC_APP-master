...
@Component({
	selector: 'app-twd-to-foreign-edit',
	templateUrl: './twd-to-foreign-edit.component.html',
	styleUrls: [],
	providers: [ DateService, ExchangeService, TwdTransferService]
})
export class TwdToForeignEditComponent implements OnInit {
	@Output() backPageEmit: EventEmitter<any> = new EventEmitter<any>();

	showNextPage = false;
	searchExchangeFlag = '';     //傳給子層返回上一頁
	...
	ngOnInit() {
		...
		this.bargain_req.negotiatedRate = "";
		this.bargain_req.negotiatedNo = "";
		...
		this._mainService.getData('B', this.formObj.openNightChk).then(
			(resObj) => {
				...
				if (this.trnsInAccts.length <= 0 || this.trnsOutAccts.length <= 0) {
					...
					/** 20190627 修改 start */
					this.propertyTypeTwToForeign = this._mainService.getPropertyTypeTwToForeign();
					...
					// 為了帳號餘額
					this.twdTransferService.getBalanceData().then(
						(result) => {
							// 放可用餘額
							...
							if (this.trnsOutAccts.length == 1) {
								let trnsAct = this.trnsOutAccts[0]['trnsfrOutAccnt'].replace(/-/g, '');
				...
				// * 為了帳號餘額
				// */
				// this.twdTransferService.getData().then(
				// 	(result) => {
				// 		// 放可用餘額
				// 		result.balance.data.forEach(element => {
				// 			this.info_other_data[element.acctNo] = element.balance;
	...
	getRate(): Promise<any> {
		...
		if (this.rate_radio.options == '1') {
			this.logger.log("走議價匯率");
			
			this.logger.log("formObj",this.formObj);
			return this._mainService.getBargainRate(this.formObj).then(
				(resObj) => {
					...
					if (this.curr_radio.options == '1') {
						...
						this.formObj.trnsfrInCurr = this.selectedCurr;
			...
			return this._mainService.getRate(this.formObj).then(
				(resObj) => {
					...
					// if (parseInt(resObj.info_data.trnsfrOutAmount) < 3000) {
					//     this.errorObj.trnsfrAmount = "交易金額不得小於TWD 3,000";
					// } else if (parseInt(resObj.info_data.trnsfrOutAmount) > 499999) {
					//     this.errorObj.trnsfrAmount = "交易金額不得大於TWD 499,999";
					// }
					...
					if (this.curr_radio.options == '1') {
						...
						this.formObj.trnsfrInCurr = this.selectedCurr;
						this.formObj.trnsfrOutAmount = resObj.info_data.trnsfrOutAmount;
						// if(this.selectedCurr=='JPY'){	//台轉外 外幣為日幣時特別處理
						// 	this.formObj.trnsfrInAmount = Math.round((this.trnsfrAmount) / parseFloat(this.formObj.trnsfrRate)).toString();
						// }else{
						// 	this.formObj.trnsfrInAmount = (parseInt(this.trnsfrAmount) / parseFloat(this.formObj.trnsfrRate)).toFixed(2).toString();
						// }
						this.formObj.trnsfrInAmount = resObj.info_data.trnsfrInAmount;
					} else {
						//固定外幣
						...
						// 	this.formObj.trnsfrOutAmount = Math.round(parseFloat(this.trnsfrAmount) * parseFloat(this.formObj.trnsfrRate)).toString();
						// }else{
						// 	this.formObj.trnsfrOutAmount = (parseFloat(this.trnsfrAmount) * parseFloat(this.formObj.trnsfrRate)).toFixed(2).toString();
						// }
						this.formObj.trnsfrOutAmount = resObj.info_data.trnsfrOutAmount;
	...
	onNextPage() {
		...
		//F5000102取得
		this.formObj.trnsfrOutCurr = 'TWD'; //轉出幣別
		this.formObj.trnsfrInCurr = this.selectedCurr;  //轉入幣別
	...
	onChangeInAcct() {
		...
		if (this.trnsInAcctsObj.hasOwnProperty('trnsInCurr')) {
			this.trnsInCurr = this.trnsInAcctsObj['trnsInCurr'];
			// this.trnsInCurr.unshift ('請選擇轉入幣別');
		}
		...
		if (this.trnsInAcctsObj == '') {
			if (this.trnsInAccts.length != 1) {
				...
				this._handleError.handleError({
					type: 'dialog',
					title: '',
					content: "請先選擇轉入帳號，轉存幣別因轉入帳號的不同，有不同的設定。"
				});
			} else {
				//轉入只有一筆時處理
				this.trnsInCurr = this.trnsInAccts[0]['trnsInCurr'];
				this.new_table = [];
	...
	toAcctArry(oldArr) {
		...
		oldArr.forEach(element => {
			if (element.trnsOutCurr.includes('TWD')) {
				newArr.push(element);
			}
		});
		return newArr;
	...
	chgBalance() {
		if (this.trnsOutAcctsObj != '') {
			let trnsAct = this.trnsOutAcctsObj['trnsfrOutAccnt'].replace(/-/g, '');
			this.balance = this.info_other_data[trnsAct];
		}
	}
	...
	onCheckEvent() {
		...
		if (this.selectedCurr == '') {
			this.errorObj.selectedCurr = "請選擇轉入幣別";
		} else {
		...
		if (this.note_input != "" && this.note_input != undefined) {
			let checkNotLen = this._mainService.checkNotLen(this.note_input);
	...
	backExchange(e) {
		...
		this._headerCtrl.updateOption({
			'title': 'FUNC_SUB.FOREX.TWD_TO_FOREX'
		});
		this._headerCtrl.setLeftBtnClick(() => {
			this.cancelEdit();
		});
	...
	//查詢結購用途說明
	searchUse() {
		// this.searchExchangeFlag = 'use';
		let purchaseUse = this.systemParameterService.get('MSG_F5000103_3_URL');
	...
	chgBargin() {
		...
		this._mainService.getBargain(req_bargin).then(
			(result) => {
