...
export class ContactBranchComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        if (this.action == 'back') {
            ...
            if (this.type == 'credit' && this.resver == 'N') {
                this.fullData.txkind = 'B';
                this.backFlag = true;
                //房貸增貸    
            } else if (this.type == 'mortgage' && this.resver == 'N') {
                this.fullData.txkind = 'A';
                this.backFlag = false;
                //房貸增貸之原案件電文，委託扣款帳號欄位為空，發新查詢帳號電文(顯示下拉)
                //查無委託帳號
                if (this.fullData.outLargn == '' || this.fullData.outLargn == null
                    || typeof this.fullData.outLargn == 'undefined') {
                    this._logger.log("has not outLargn");
                    this.doApi505().then(
                        (success) => {
                            this.hasAccount = false;
                            this.dfAccountFlag = false;
                        },
                        (failed) => {
                            this._logger.log("get 505 account error");
                        }
                    );

                    //有委託扣款帳號 
                } else {
                    this._logger.log("has outLargn");
                    this.hasAccount = true;
                    this.dfAccountFlag = true; //顯示存款科目、餘額欄位
                    //如果查的到帳號資料，直接帶入上一頁資料
                    this.bindData.accountcd = this.fullData.accountcd;
                    ...
                    this.bindData.outLargn = this.fullData.outLargn;
                    this.bindData.outGnamt = this.fullData.outGnamt;
                    this.bindData.outBrch2 = this.fullData.outBrch1;
                    this.bindData.outBrchName2 = this.fullData.outBrchName2;
                }
                //房屋貸款(預填單)
            } else if (this.type == 'house' && this.resver == 'Y') {
                this.fullData.txkind = 'D';
                this.backFlag = true;
                //預填單只有自行選擇
    ...
    //---------- 查無帳號料相關 START----------
    //發新api f9000505
    doApi505(): Promise<any> {
        let reqData_505 = {
            custId: ''
        };
        return this._mainService.getAccount(reqData_505).then(
            (result) => {
                this._logger.log("success, result:", result);
                this.accountData_505 = result.data; //505帳號資料
                this._logger.log("accountData_505:", this.accountData_505);
    ...
    onNext3() {
        ...
        if (this.type == 'mortgage') {
            saveData = this.bindData;
            saveData.txkind = this.fullData.txkind;
            type = 'mortgage';
            //為了返回暫存資料機制，暫存資料
            this._allService.setHouseBranchData(saveData);
            //存取分行狀態(有無威托扣款帳號)
            this._allService.setBranchStatus(this.hasAccount);
        } else {
            //信貸
            saveData.txkind = this.fullData.txkind;
                    ...
                    if (this.resver == 'N') {
                        ...
                        if (this.checkCaptchaFlag == true) {
                            ...
                            if (reqData['txkind'] == null || typeof reqData['txkind'] == 'undefined' || reqData['txkind'] == '') {
                                ...
                                if (this.type == 'mortgage') {
                                    reqData['txkind'] = 'A'; //房貸增貸
                                } else if (this.type == 'credit') {
                                    reqData['txkind'] = 'B'; //信貸
                                } else {
                                    reqData['txkind'] = 'D'; //房屋貸款
                                }
                            }
                            //formate金額...等
 ...
 */
    ...
    setOutputData(stage) {
        ...
        if (this.type == 'credit') {
            ...
            this._logger.log("into setOutputData type mortgage");
            let branchData = this._allService.getHouseBranchData();
            this.bindData.outLaran = branchData['outLaran'];
            this.bindData.outBrch1 = branchData['outBrch1'];
            this.bindData.outBrchname1 = branchData['outBrchname1'];
            this.bindData.outLarpm = branchData['outLarpm'];
            this.bindData.outLaral = branchData['outLaral'];
            this.bindData.increaseAmount = branchData['increaseAmount'];
            this.bindData.outRate = branchData['outRate'];
            this.bindData.outLargn = branchData['outLargn'];
            this.bindData.outBrch2 = branchData['outBrch2'];
            this.bindData.outBrchName2 = branchData['outBrchName2'];
            this.bindData.accountcd = branchData['accountcd'];
            this.bindData.outGnamt = branchData['outGnamt'];
            // let accountData = this._allService.getAccountBind();
            // this.accountBind = accountData;
            let branchStatus = this._allService.getBranchStatus();
            this.hasAccount = branchStatus;
            this.dfAccountFlag = true;
            if (this.hasAccount == false) {
                this.doApi505();
            }
            this._logger.log("setOutputData mortgage, hasAccount:", this.hasAccount);
            this._logger.log("setOutputData mortgage, branchStatus:", branchStatus);
            if (stage == true) {
                this.nowPage = 'commission';
            } else {
                this.nowPage = 'confirmPage';
