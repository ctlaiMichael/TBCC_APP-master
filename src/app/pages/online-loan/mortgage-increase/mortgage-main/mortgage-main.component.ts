 ...
 * 線上申貸-房貸增貸-主頁
 * 
 * 流程： 分行=>同意選單=>KYC=>申請書=>同一關係人=>文件上傳=>結果
 */
...
@Component({
    selector: 'app-mortgage-main',
    templateUrl: './mortgage-main.component.html',
    styleUrls: [],
    providers: [PaginatorCtrlService]
})

export class MortgageMainComponent implements OnInit {
    ...
    ngOnInit() {
        this._logger.log("into mortgage-main page");
        //重置全部暫存資料
        this.restAllData();
        // 判斷該使用者是否有憑證或OTP。
        let OTPInfo = this._authService.checkAllowOtp(true);
        let CAInfo = this._authService.checkDownloadCA(true);
        if (OTPInfo == false && CAInfo == false) {
            this._handleError.handleError({
                type: 'message',
                title: '訊息',
                content: "本項服務需使用「憑證」或「OTP」轉帳機制始得進行交易，請洽本行各營業單位辦理。"
            });
            return false;
        }
        //發送f9000401 取得基本資料
    ...
    */
    onPageBackEvent(e) {
        ...
        if (typeof e === 'object') {
            ...
            if (e.hasOwnProperty('intoFirst')) {
                intoFirst = e.intoFirst;
            }
            //前往下一頁
            if (pageType == 'go') {
                switch (page) {
                    case 'contact-branch': //選擇分行(回傳) Go
                        ...
                        this.agreeStep = 'another'; //不走簡介
                        this.onChangePage('agree-term'); //前往同意選單
                        break;
                    ...
                    case 'relation-form': //同一關係人(回傳) Go
                        ...
                        this.fullData = tmp_data;
                        this.action = 'go';
                        this.onChangePage('file-upload'); //前往上傳
                        break;
                ...
                switch (page) {
                    ...
                    case 'agree': //同意選單(回傳) Back
                        ...
                        this.fullData = tmp_data;
                        this.action = 'back';
                        this.onChangePage('contact-branch'); //返回原案件
                        break;
                    case 'kyc': //kyc(回傳) Back
                        this._logger.log("into kyc back main");
                        this.fullData = tmp_data;
                        this.action = 'back';
                        this.agreeStep = 'another'; //顯示契約
                    ...
                    case 'relation-form': //同一關係人(回傳) Back
                        ...
                        this._logger.log("into relation-form back main, tmp_data:", tmp_data);
                        this.fullData = tmp_data;
                        this._logger.log("into relation-form back main, fullData:", this.fullData);
                        this.action = 'back';
                ...
                switch (page) {
                    case 'agree-first': //簡介(回傳)  pageType == 'first-go'
                        ...
                        this.action = 'go';
                        this.showContent = false;
                        this.onChangePage('branch-case'); //前往原案件
                        break;
                    case 'list-item': //原案件分頁回傳 pageType === 'page_info'
                        ...
                        break;
                    case 'branch-case': //點擊增貸按鈕接收(回傳) pageType == 'go-branch'
                        this._logger.log("into branch-case go main");
                        this.fullData = tmp_data;
                        this.action = 'go';
