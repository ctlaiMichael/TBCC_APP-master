(function(QRCode){
  ...
  .directive('qr', ['$timeout', '$window', function($timeout, $window){
    ...
    return {
      restrict: 'E',
      template: '<img ng-src="{{LOGO_SRC}}" ng-if="LOGO_SRC" style="\
      position: absolute;\
      margin-top: {{OFFSET}}px;\
      margin-left: {{OFFSET}}px;\
      width: {{LOGO_SIZE}}px;\
      height: {{LOGO_SIZE}}px;\
  "><canvas ng-hide="image"></canvas><img ng-if="image" ng-src="{{canvasImage}}"/>',
      scope: {
        typeNumber: '=',
        correctionLevel: '=',
        inputMode: '=',
      ...
      link: function postlink(scope, element, attrs){
        ...
        scope.canvasImage = '';
        scope.LOGO_SRC = scope.getLogoSrc();
        scope.LOGO_SIZE = scope.SIZE/5;
        scope.OFFSET = (scope.SIZE-scope.LOGO_SIZE)/4;
