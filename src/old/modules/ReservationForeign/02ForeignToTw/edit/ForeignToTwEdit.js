...
define([
	...
	//=====[ START]=====//
	MainApp.register.controller('ForeignToTwEditCtrl',
		function (
			...
			var config =[];
			$scope.config =["0000結售之外匯原係以新台幣結購存入","111E海運貨運收入","112E海運客運收入","115E航空貨運收入","116E航空客運收入","122E財產保險理賠收入","191E文化及休閒收入","192E貿易佣金及代理費收入","193E營建收入","195E使用智慧財產權收入","19AE郵務與快遞收入","19BE電腦與資訊收入","19CE營業租賃收入","19DE專業技術事務收入","19EE視聽收入","210A收回股本投資","220B收回貸款投資","250A收回國外存款","262A收回投資國外股權證券","263A收回投資國外長期債票券","264A收回投資國外短期債票券","266A國外有本金交割的遠匯及換匯之資金匯入","267A國外無本金交割的衍生金融商品之資金匯入","270A收回投資國外不動產","280B收回對外貸款","291Z收回分期付款出口融資","292Z資本租賃收入","340B國外借款","370A外人購置不動產","410C薪資款匯入","440C對外貸款利息","441C股本投資盈餘或股利","442C股權證券股利","443C國外存款利息","444C有關出口之利息","445C長期債票券利息","446C短期債票券利息","448C貸款投資利息","510F贍家匯款收入","511G工作者匯款收入","520F捐贈匯款收入","540F出售自然資源與非研發成果資產收入","612Z旅行剩餘退匯","70AI收款人已自行辦理出口通關的貨款","701I尚未出口之預收貨款","702I港口售油及補給","704I樣品費收入","710D委外加工貿易收入","711D商仲貿易收入"];
			...
			$scope.clickOutAccnt = function () {
				
				//alert($scope.life);
				//alert($scope.InAC[$scope.life].accCurr);
				var ww= $scope.InAC[$scope.life].accCurr.split(',');
				//alert(ww[0]);
				$scope.coin =ww ;
					...
					if (res) {
						...
						for (key in actsOut) {
								...
								if(actsOut[key].trnsOutCurr.indexOf("TWD") == -1){
									...
									tempIn.accCurr = actsOut[key].trnsOutCurr;
			...
			$scope.clickSubmit = function () {
				...
				//注意事項
					...
					framework.alert("訊息長度有誤");
						...
						//alert($scope.status);
						if($scope.color.name== 2){$scope.form9.trnsAmount=$scope.trnsAmountStr;}
						else{$scope.form9.trnsAmount = $scope.trnsAmountStr1+"."+$scope.trnsAmountStr2;}
						if($scope.color.name==2){$scope.form9.tcoin="TWD";}
						...
						$state.go('ForeignToTwCheck',params,{location: 'replace'});
