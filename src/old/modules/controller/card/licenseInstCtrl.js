...
define([
	...
	,'directive/licenseInstDirective'
	//==Services==//
	,'service/loginServices'
	,'service/formServices'
	,'service/payCardFeeServices'
	,'service/licenseServices'
],function(MainApp){
 ...
 */
MainApp.register.controller('licenseInstCtrl',function(
	$scope,$state,$element,$compile,i18n
	,loginServices,formServices,sysCtrl
	,payCardFeeServices,licenseServices
){
	$scope.inp = {};
	$scope.curStep = 0;
	var haveStep = 1;
	...
	$scope.onAgree = function(){
		...
		if(check_flag){
			licenseServices.getErrorMsg('AGREE_PROVISION_ONE');
			return false;
		}
		$scope.returnStepView(2,"bill-inst-form-directive");
		$scope.billOptions = licenseServices.getBillOptions(true);
		//debugger;
	}
	...
	$scope.onForm = function(){
		if(typeof $scope.selectBill !== 'object'){
			licenseServices.getErrorMsg('FC0009_002');
			return false;
		}
	...
	var checkCaptch = function(){
		...
		if(!result){
			...
			licenseServices.getErrorMsg('INPUT_ERROR');
			return false;
		}
