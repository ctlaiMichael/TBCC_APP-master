...
define([
	...
	,'directive/taxbillInstDirective'
	//==Services==//
	,'service/loginServices'
	,'service/formServices'
	,'service/payCardFeeServices'
	,'service/taxbillServices'
],function(MainApp){
 ...
 */
MainApp.register.controller('taxbillInstCtrl',function(
	$scope,$state,$element,$compile,i18n
	,loginServices,formServices,sysCtrl
	,payCardFeeServices,taxbillServices
){
	$scope.inp = {};
	$scope.curStep = 0;
	var haveStep = 1;
	...
	$scope.onAgree = function(){
		...
		if(check_flag){
			taxbillServices.getErrorMsg('AGREE_PROVISION_ONE');
			return false;
		}
		$scope.returnStepView(2,"bill-inst-form-directive");
		$scope.billOptions = taxbillServices.getBillOptions(true);
		//debugger;
	}
	...
	$scope.onForm = function(){
		if(typeof $scope.selectBill !== 'object'){
			taxbillServices.getErrorMsg('FC0009_002');
			return false;
		}
		...
		MainUiTool.openConfirm({
			...
			content : i18n.getStringByTag('CTRL_INSTALLMENT.MSG.NOTE_3'),
			success : function(){
				// $scope.signatureSet = {
	...
	var checkCaptch = function(){
		...
		if(!result){
			...
			taxbillServices.getErrorMsg('INPUT_ERROR');
			return false;
		}
