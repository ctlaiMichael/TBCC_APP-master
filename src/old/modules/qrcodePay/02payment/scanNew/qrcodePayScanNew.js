...
define([
	...
	//=====[ START]=====//
	MainApp.register.controller('qrcodePayScanNewCtrl',
		function (
		...
		(function () {
			...
			//==參數設定==//
			$scope.nowScreen = "qrcodePayScanNew";
			...
			var callback_method = function(success,resultObj){
				...
				qrcodePayServices.getLoginInfo(function(res){
					...
					if (securityTypes.length > 0) {
						...
						else if(resultObj.data.acqInfo.substring(0,3)=='11,'){
							//檢查是否有OTP
							// alert("acqInfo"+resultObj.data.acqInfo);
							// alert(JSON.stringify(res));
							qrcodePayServices.getLoginInfo(function (res) {
								$scope.custId = res.custId;
								if (!(res.BoundID == 4 && res.OTPID == 2 && res.AuthStatus == 0 && res.PwdStatus == 0) ) {
										framework.alert('您的行動裝置無(OTP)機制服務，請洽營業單位申請。', function(){
											qrcodePayServices.closeActivity();
										});
									}
								else{
			...
			function getFQ000420() {
				...
				qrCodePayTelegram.send('qrcodePay/fq000420', form, function (res, error) {
					console.log("fq000420 res:" + JSON.stringify(res));
					//res={"cixinFlg":"0","cixP33":"","status": true,"cName": "李先生","cGdpr": "中華民國","custId": "A123456789","birthDate": "771129","phoneMobile":"0987654321"};
					if (res) {
						//alert(JSON.stringify(resultObj11));
						if(res.cixp3Cid != 'Y' || res.cixP33Flg != 'Y'){
							var params = {
								result:res
							};
							$state.go('outboundAgree',params,{location: 'replace'});
						}
						else{$state.go('qrCodePayForm2',{  
							trnsfrOutAcct:$scope.defaultTrnsOutAcct,
							trnsLimitAmt:$scope.trnsLimitAmt,
							qrcode:resultObj11.data},{location: 'replace'});}
